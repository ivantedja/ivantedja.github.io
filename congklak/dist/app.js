...
var Board = _interopRequire(require("./model/Board"));

var Congklak = (function () {
    function Congklak() {
        _classCallCheck(this, Congklak);
    ...
    _createClass(Congklak, {
        ...
        printBoard: {
            value: function printBoard() {
                ...
                var leftPadding = function (str) {
                    return ("    " + str).slice(-4);
                };

                var str = "Current player: " + this.board.getCurrentPlayer().id + "\n";
                str += leftPadding("");
                for (var i = this.board.numOfHolesAndStones + 1; i <= 2 * this.board.numOfHolesAndStones; i++) {
                    str += leftPadding(this.board.holes[i].stones.length);
                }
                str += "\n";

                str += leftPadding(this.board.getPlayerSouthStoreHouse().stones.length);
                for (var i = 0; i < this.board.numOfHolesAndStones; i++) {
                    str += leftPadding("");
                }
                ...
                str += leftPadding("");
                for (var i = this.board.numOfHolesAndStones - 1; i >= 0; i--) {
                    str += leftPadding(this.board.holes[i].stones.length);
                }
                str += "\n----------------------------\n";
        ...
        getBoardHtml: {
            value: function getBoardHtml() {
                var html = "Current Player: " + this.board.getCurrentPlayer().id;
                ...
                html += "<tr>";
                html += "<td><span class=\"store-house\" id=\"hole-" + this.board.getPlayerSouthStoreHouseIndex() + "\">" + this.board.getPlayerSouthStoreHouse().stones.length + "</span></td>";
                html += "<td colspan=\"" + this.board.numOfHolesAndStones + "\"></td>";
                html += "<td><span class=\"store-house\" id=\"hole-" + this.board.getPlayerNorthStoreHouseIndex() + "\">" + this.board.getPlayerNorthStoreHouse().stones.length + "</span></td>";
                html += "</tr>";
        ...
        observeOnClickHole: {
            value: function observeOnClickHole() {
                if (this.isNonBrowser()) {
                    return;
                }
                var self = this;
                $(".hole").each(function (idx, elem) {
                    $(elem).click(function () {
        ...
        showRoundControls: {
            value: function showRoundControls() {
                if (this.isNonBrowser()) {
                    return;
                }
                $("#round-controls").show();
                $("#round-controls").find("button").each(function (idx, elem) {
                    $(elem).click(function () {
                        $("#round-controls").hide();
                    });
                });
        ...
        checkGameOverAndEnd: {
            value: function checkGameOverAndEnd() {
                // debug
                this.printBoard();
                this.drawBoardUI();
                ...
                if (this.board.isAnyPlayerHolesEmptyAndSetNextPlayer()) {
                    ...
                    var playerSouthStoreHouse = this.board.getPlayerSouthStoreHouse();
                    var playerNorthStoreHouse = this.board.getPlayerNorthStoreHouse();
                    var playerSouthStones = playerSouthStoreHouse.stones.length;
                    var playerNorthStones = playerNorthStoreHouse.stones.length;
                    var winner = null;
        ...
        endRound: {
            value: function endRound(winner) {
                this.gameStatus = 1;
        ...
        chooseHoleAndDistribute: {
            value: function chooseHoleAndDistribute(holeIndex) {
                var isHumanChoice = arguments[1] === undefined ? true : arguments[1];
                ...
                var startHole = this.board.holes[holeIndex];
                ...
                var stonesInHand = startHole.takeAllStones();
                // debug
                this.printBoard();
                ...
                while (stonesInHand.length > 0) {
                    ...
                    var nextHoleIndex = this.board.getNextHoleIndex(holeIndex);
                    var nextHole = this.board.holes[nextHoleIndex];
                ...
                // record last hole
                var lastHoleIndex = holeIndex;
                var lastHole = this.board.holes[lastHoleIndex];
                ...
                if (lastHole.isStoreHouse && lastHole.player.id === currentPlayer.id) {
                    ...
                    var oppositeHoleIndex = this.board.getOppositeHoleIndex(lastHoleIndex);
                    var oppositeHole = this.board.holes[oppositeHoleIndex];
                    if (oppositeHole.stones.length > 0) {
                        // tembak
                        this.showMessage("Tembak!");
                        var currentPlayerStoreHouse = this.board.getCurrentPlayerStoreHouse();
                        // retrieve stones from current last hole
                        var lastHoleStones = lastHole.takeAllStones();
                        currentPlayerStoreHouse.addStones(lastHoleStones);
                        // retrieve stones from opposite holes
                        var oppositeHoleStones = oppositeHole.takeAllStones();
...
module.exports = Congklak;

},{"./model/Board":3,"jquery":2}],2:[function(require,module,exports){
/*!
...
var Hole = _interopRequire(require("./Hole"));

var StoreHouse = _interopRequire(require("./StoreHouse"));

var Stone = _interopRequire(require("./Stone"));
...
var Board = (function () {
    ...
    _createClass(Board, {
        ...
        initNextRound: {
            value: function initNextRound() {
                this.cleanupStones();
                this.holes = this.generateHolesNextRound();
            }
        },
        generateHolesNextRound: {
            value: function generateHolesNextRound() {
                // iterate player, assumed always:
                //  - [0] -> south player
                //  - [1] -> north player
                var holes = [];
                for (var i = 0; i < this.players.length; i++) {
                    var playerStoreHouse = i === 0 ? this.getPlayerSouthStoreHouse() : this.getPlayerNorthStoreHouse();
                    var storeHouseStones = playerStoreHouse.stones.length;
                    var holesFilled = Math.min(Math.floor(storeHouseStones / this.numOfHolesAndStones), this.numOfHolesAndStones);
                    var holesBlocked = this.numOfHolesAndStones - holesFilled;
                    var storeHouseStonesInit = storeHouseStones - holesFilled * this.numOfHolesAndStones;
                    var offset = i === 0 ? 0 : this.numOfHolesAndStones + 1;
                    // iterate holes
                    for (var j = offset; j < offset + this.numOfHolesAndStones; j++) {
                        // hole
                        var hole = new Hole(this.players[i]);
                        // some holes need to be blocked
                        if (holesBlocked > 0) {
                            hole.isBlocked = true;
                            holesBlocked--;
                        } else {
                            var _stones = this.generateStones();
                            hole.addStones(_stones);
                        }
                        // add hole to list
                        holes.push(hole);
                    }
                    // store house
                    var storeHouse = new StoreHouse(this.players[i]);
                    var stones = this.generateStones(storeHouseStonesInit);
        ...
        setNextRoundPlayer: {
            value: function setNextRoundPlayer(playerIndex) {
                this.currentPlayerIndex = playerIndex;
            }
        },
        generatePlayers: {
            value: function generatePlayers() {
                var playerA = new Player("south");
                var playerB = new Player("north");
        ...
        generateStones: {
            ...
            value: function generateStones() {
                var numOfStones = arguments[0] === undefined ? this.numOfHolesAndStones : arguments[0];
        ...
        generateHoles: {
            ...
            value: function generateHoles() {
                ...
                for (var i = 0; i < this.players.length; i++) {
                    // iterate holes
                    for (var j = 0; j < this.numOfHolesAndStones; j++) {
                        // hole
                        var hole = new Hole(this.players[i]);
                        var stones = this.generateStones();
        ...
        getNextHoleIndex: {
            ...
            value: function getNextHoleIndex(holeIndex) {
                var nextHoleIndex = holeIndex >= this.getPlayerNorthStoreHouseIndex() ? 0 : holeIndex + 1;
                var nextHole = this.holes[nextHoleIndex];
        ...
        getOppositeHoleIndex: {
            value: function getOppositeHoleIndex(holeIndex) {
                return 2 * this.numOfHolesAndStones - holeIndex;
            }
        },
        isPlayerSouthHolesEmpty: {
            value: function isPlayerSouthHolesEmpty() {
                var sum = 0;
                for (var i = 0; i < this.numOfHolesAndStones; i++) {
        ...
        isPlayerNorthHolesEmpty: {
            value: function isPlayerNorthHolesEmpty() {
                var sum = 0;
                for (var i = this.numOfHolesAndStones + 1; i < 2 * this.numOfHolesAndStones + 1; i++) {
                    sum = sum + this.holes[i].stones.length;
                }
                return sum === 0 ? true : false;
            }
        ...
        isAnyPlayerHolesEmptyAndSetNextPlayer: {
            value: function isAnyPlayerHolesEmptyAndSetNextPlayer() {
        ...
        cleanupStones: {
            value: function cleanupStones() {
                // south
                var playerSouthStoreHouse = this.getPlayerSouthStoreHouse();
                for (var i = 0; i < this.numOfHolesAndStones; i++) {
                    playerSouthStoreHouse.addStones(this.holes[i].takeAllStones());
                }
                // north
                var playerNorthStoreHouse = this.getPlayerNorthStoreHouse();
                for (var i = this.numOfHolesAndStones + 1; i < 2 * this.numOfHolesAndStones + 1; i++) {
...
},{"./Hole":4,"./Player":5,"./Stone":6,"./StoreHouse":7}],4:[function(require,module,exports){
"use strict";
...
var Hole = (function () {
    ...
    _createClass(Hole, {
        addStone: {
            value: function addStone(stone) {
                this.stones.push(stone);
            }
        },
        addStones: {
            value: function addStones(stones) {
                this.stones = this.stones.concat(stones);
            }
        },
        takeAllStones: {
            value: function takeAllStones() {
...
var StoreHouse = (function (_Hole) {
    function StoreHouse(id) {
        _classCallCheck(this, StoreHouse);

        _get(Object.getPrototypeOf(StoreHouse.prototype), "constructor", this).call(this, id);
        this.isStoreHouse = true;
    }

    _inherits(StoreHouse, _Hole);
